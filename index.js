...
class Car {
  ...
  drive(distance){
    ...
    if(distance <= driveMiles){
      ...
      this.odometr = this.odometr + driveMiles;
      this.tank = 0;
      return `I ran out of fuel al ${this.odometr} miles`
    }
  }
}
...
class ProjectManager  extends Instructor{
  ...
  standUp(channel){
    return `${this.name} annouces to ${channel},@chanel standy times!`
  }
  debugCode(subject){
    return `${this.name} debugs ${subject}`;
  }
}
